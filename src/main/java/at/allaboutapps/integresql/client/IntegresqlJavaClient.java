...
public class IntegresqlJavaClient implements AutoCloseable {
    ...
    /**
     * Creates a new client using default configuration loaded from environment variables.
    ...
    private <T> T parseJsonResponse(HttpResponse<String> response, Class<T> responseType) throws IOException {
        ...
        try {
            ...
            throw new IOException("Failed to parse JSON response body: " + e.getMessage() + "; Body snippet: " + bodySnippet, e);
        }
    }

    private <T> ResponseWrapper<T> doRequestInternal(String method, String endpoint, Object requestBody, Class<T> responseType) {
        ...
        try {
            ...
            if (response.statusCode() != 202 && response.statusCode() != 204 && responseType != null && responseType != Void.class) {
     ...
     * If the template is already being prepared or is ready, specific exceptions are thrown.
     * Corresponds to POST /templates.
     *
     * @param hash The template hash.
     * @return Details of the template database being initialized.
     * @throws TemplateAlreadyInitializedException if initialization is already in progress or finished.
     * @throws ManagerNotReadyException if the manager service is unavailable.
     * @throws IntegresqlException      for other errors (network, unexpected status).
     */
     ...
     * @throws TemplateNotFoundException if the template hash is not found (e.g., never initialized or discarded).
     * @throws ManagerNotReadyException if the manager service is unavailable.
     ...
     * It calls {@link #initializeTemplate(String)}. If successful, it runs the provided
     * initializer function with the template database's connection string, and then calls
     * {@link #finalizeTemplate(String)}. If initialization fails, it attempts to discard
     * the template. If the template was already initialized, it does nothing.
     *
     * @param hash        The template hash.
     * @param initializer A consumer function that takes the JDBC connection string to perform setup (e.g., run migrations).
     * Any exception thrown by the initializer will cause the setup to fail and the template to be discarded.
     * @throws Exception If any step fails (initialization, initializer function, finalization, discard).
     * Specific exceptions like ManagerNotReadyException might be thrown.
     */
    public void setupTemplate(String hash, Consumer<String> initializer) throws Exception {
        ...
        try {
            ...
            try {
                DatabaseConfig config = Objects.requireNonNull(template.database.config, "Template database config is null");
                ...
                try {
                    ...
                    System.err.println("Failed to discard template " + hash + " after initializer failure: " + discardEx.getMessage());
    ...
    /**
     * High-level method similar to {@link #setupTemplate(String, Consumer)}, but provides
     ...
     *
     * @param hash        The template hash.
     * @param initializer A consumer function that takes the JDBC {@link Connection} to perform setup.
     * Any exception thrown by the initializer will cause the setup to fail and the template to be discarded.
     * @throws Exception             If any step fails (initialization, DB connection, initializer function, finalization, discard).
    ...
    public void setupTemplateWithDBClient(String hash, Consumer<Connection> initializer) throws Exception {
        ...
        try {
            ...
            throw new IllegalStateException("PostgreSQL JDBC driver not found on classpath. Please add 'org.postgresql:postgresql' dependency.", e);
        }
        ...
        try {
            ...
            } catch (Exception e) { // Catch SQLException from getConnection/isValid or any exception from initializer
                System.err.println("Initializer or DB connection failed for template " + hash + ", attempting to discard.");
                try {
                    discardTemplate(hash);
                } catch (Exception discardEx) {
                    System.err.println("Failed to discard template " + hash + " after failure: " + discardEx.getMessage());
    ...
    /**
     * Gets a unique, isolated test database based on a previously finalized template hash.
     ...
     * @throws TemplateNotFoundException if the template hash is not found or not finalized.
     * @throws DatabaseDiscardedException if the template database was discarded during setup.
     * @throws ManagerNotReadyException if the manager service is unavailable.
     * @throws IntegresqlException      for other errors.
     */
    public TestDatabase getTestDatabase(String templateHash) {
        ...
        return switch (response.statusCode) {
            case 200 -> // OK
                    response.body;
            case 404 ->
                    throw new TemplateNotFoundException("Template not found or not finalized for getTestDatabase: " + templateHash);
            case 410 ->
                    throw new DatabaseDiscardedException("Template database was discarded, cannot get test database for hash: " + templateHash);
            case 503 -> throw new ManagerNotReadyException("IntegreSQL manager service is not ready.");
            default ->
     ...
     * @param databaseId   The unique ID (or hash) of the test database instance (from TestDatabase.databaseHash).
     * @throws TemplateNotFoundException if the template hash is not found.
     * @throws TestNotFoundException     if the specific test ID is not found for that template.
     * @throws ManagerNotReadyException  if the manager service is unavailable.
     * @throws IntegresqlException       for other errors.
     */
    public void returnTestDatabase(String templateHash, Integer databaseId) {
        ...
        // Go client used int ID, but DTOs suggest ID might be the hash string. Adjust if API uses int.
        ...
        switch (response.statusCode) {
            case 204: // No Content
                return; // Success
            case 404: // Not Found
                throw new TemplateNotFoundException("Template or Test Database not found for return: template=" + templateHash + ", dbId=" + databaseId);
            case 503: // Service Unavailable
                throw new ManagerNotReadyException("IntegreSQL manager service is not ready.");
            default:
                throw new IntegresqlException("Return test database failed with unexpected status", response.statusCode);
        }
    }
    ...
    public void close() {
        ...
        // If using default executor, it might be shared. Explicit shutdown usually not needed.
        System.out.println("IntegresqlJavaClient close() called - Note: HttpClient cleanup might be managed elsewhere or automatic.");
    }
}